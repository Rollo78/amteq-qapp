...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        if (serverUrl == null || serverUrl.trim().length() == 0 || serverUrl.startsWith("http://10.0.0.74")) {
            serverUrl = DEFAULT_URL;
            sp.edit().putString("url", DEFAULT_URL).apply();
        }
        setupWebView();
        showLoadingScreen("Verbinde mit AMTEQ Q-App Server...", serverUrl);
    ...
    private void setupWebView(){
        ...
        web.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView v, String url){ errorScreenVisible = false; }
            @Override public void onReceivedError(WebView v, int code, String desc, String failingUrl){ showAutoRetryScreen(serverUrl); }
        });
    ...
    private void showAutoRetryScreen(String current){
        if(errorScreenVisible) return;
        errorScreenVisible = true;
        LinearLayout l = baseLayout();
        l.addView(makeText("AMTEQ Q-App", 28, 18));
        l.addView(makeText("Server nicht erreichbar\n\nDie App verbindet automatisch erneut.\nServer: " + current, 17, 26));
        l.addView(new ProgressBar(this));
